(function (ng) {
  ...
  /**
   * @ngdoc directive
   * @name ngCrud.directive:ifRol
   * @priority 0
   * @restrict A // Only as attribute
   * @scope
   * 
   * @param {String} name: names of the permitted rol separated by space ' '
   * 
   * @description Hides element when rol is not permitted
   */
  mod.directive('ifRol', ['CrudTemplatesDir', 'authService', function (tplDir, authService) {
      function link(scope, element, attrs) {
        var roles = authService.getRoles();//Get roles of the user
        var askedRoles = scope.name.split(' ');//separates input roles into an array
        var haveRol = false;//Util variable
        for(var rol in askedRoles){//iterates all the input roles
          if (roles.indexOf(rol) !== -1) {//if has the rol change the variable to true
            haveRol = true;
          }
        }
        if(!haveRol){//if don't have any of the roles hides the element
